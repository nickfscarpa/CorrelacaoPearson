...
float r(float xIyI, float somaXi, float somaYi, float somaXi2, float somaYi2, int num){
    ...
    cout << fixed << setprecision(2); // Limitando as casas decimais em duas casas. | Limiting decimal places to two places.

    if (p == 1){
      cout << "\nCorrelacao perfeita positiva" << endl;
    }else if (p == -1){
      cout << "\nCorrelacao perfeita negativa" << endl;
    }else if (p >= 0 && p <= 0.3){
      cout << "\nCorrelacao nula" << endl;
    }else if (p > 0.3 && p <= 0.6){
      cout << "\nCorrelacao fraca" << endl;
    }else if (p > 0.6 && p < 1){
      cout  << "\nCorrelacao forte" << endl;
    }else{
      cout << "\nInexistente" << endl;
    }
...
int main(){
  ...
  float Yi[n];
   ...
   // Chamando as funções e armazenando nas em variaveis. | Calling functions and storing them in variables.
   pearson = r(XiYi,SomaXi,SomaYi,SomaXi2,SomaYi2,n);
